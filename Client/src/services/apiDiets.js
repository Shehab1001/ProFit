...
export async function getDietTemplates(token, page) {
  const response = await fetch(
    `http://localhost:4000/api/v1/Nutrition/Myplans/?page=${page}&limit=${PAGE_SIZE_MEALS}`,
    {
      method: "GET",
      headers: {
...
export async function getDietFreePlans(token, page) {
  const response = await fetch(
    `http://localhost:4000/api/v1/Nutrition/FreePlans?page=${page}&limit=${PAGE_SIZE_MEALS}`,
    {
      method: "GET",
      headers: {
        authorization: `Bearer ${token}`,
...
export async function getSpecificDietTemplate(token, id) {
  const response = await fetch(
    `http://localhost:4000/api/v1/Nutrition/${id}`,
    {
      method: "GET",
      headers: {
        authorization: `Bearer ${token}`,
...
export async function createDietTemplate(token, dietData) {
  const response = await fetch(
    "http://localhost:4000/api/v1/Nutrition/",
    {
      method: "POST",
      headers: {
        authorization: `Bearer ${token}`,
