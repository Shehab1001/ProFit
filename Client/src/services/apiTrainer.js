...
export async function getUserAbout(token) {
  const response = await fetch(
    "http://localhost:4000/api/v1/trainers/trainer_about",
    {
      method: "GET",
      headers: {
...
export async function updateUserAbout(userData, token) {
  const response = await fetch(
    "http://localhost:4000/api/v1/trainers/update_trainer_about",
    {
      method: "PATCH",
      headers: {
...
export async function getTrainerTranscations(token, page, filter) {
  ...
  const response = await fetch(
    `http://localhost:4000/api/v1/trainers/AllSubscriptions?page=${page}&limit=${PAGE_SIZE_DEFAULT}${filterValue}`,
    {
      method: "GET",
      headers: {
        authorization: `Bearer ${token}`,
